...
class OrchestratedConverter:
    ...
    def __init__(self, debug: bool = False):
        ...
        self.static_converter = VoxBridgeConverter(debug=debug)
        self.conversion_stats = {}
        self.extension_handler = GLTFExtensionHandler()
        self.error_handler = VoxBridgeErrorHandler()
    ...
    def convert_file(self, input_path: Path, output_dir: Path, target: str, 
        ...
        try:
            ...
            # Step 3: Route to appropriate converter with robust fallback
            if use_complex_path:
                if self.node_available:
                    logger.info("Using Node.js complex processing path")
                    try:
                        ...
                        # Check if Node.js processing was successful
                        if not result.get('success', False):
                            logger.warning("Node.js processing failed, attempting Python fallback")
                            result['fallback_used'] = True
                            result['fallback_reason'] = "Node.js processing failed"
                            result = self._process_static_file(input_path_obj, Path(output_dir), target, options, result, input_path_obj)
                            
                    except Exception as e:
                        logger.warning(f"Complex processing failed with exception: {e}, falling back to static processing")
                        result['fallback_used'] = True
                        result['fallback_reason'] = f"Complex processing exception: {e}"
                        result['error'] = f"Complex processing failed: {e}"
                        result = self._process_static_file(input_path_obj, Path(output_dir), target, options, result, input_path_obj)
                else:
                    logger.warning("Node.js not available, using Python fallback")
                    result['fallback_used'] = True
                    result['fallback_reason'] = "Node.js not available"
                    result = self._process_static_file(input_path_obj, Path(output_dir), target, options, result, input_path_obj)
            else:
                logger.info("Using Trimesh static processing path")
                result = self._process_static_file(input_path_obj, Path(output_dir), target, options, result, input_path_obj)
            
            # Step 4: Handle GLTF extensions and texture fixes
            if result['success']:
                output_path = result.get('output_path')
                if output_path and Path(output_path).exists():
                    # Handle GLTF extensions
                    if output_path.endswith('.gltf'):
                        extension_result = self.extension_handler.process_gltf_file(
                            Path(output_path), target
                        )
                        if extension_result['fallbacks_applied']:
                            result['extension_fallbacks'] = extension_result['fallbacks_applied']
                            result['warnings'].extend(extension_result['warnings'])
                            logger.info(f"Applied {len(extension_result['fallbacks_applied'])} extension fallbacks")
                    
                    # Ensure texture embedding and fix paths
                    texture_embedding_result = ensure_texture_embedding(output_path, Path(output_path).parent)
                    if texture_embedding_result['success']:
                        logger.info("Texture embedding checks completed")
                        if 'fixes applied' in texture_embedding_result['message']:
                            result['texture_fixes'] = texture_embedding_result['message']
                    
                    # Flatten texture paths for Unity/Roblox compatibility
                    texture_flatten_result = flatten_texture_paths(output_path, Path(output_path).parent)
                    if texture_flatten_result['success']:
                        logger.info("Texture path flattening completed")
                        if 'flattened' in texture_flatten_result['message']:
                            result['texture_path_fixes'] = texture_flatten_result['message']
                    
                    # Validate and fix UV maps
                    uv_validation = validate_uv_maps(output_path)
                    if uv_validation['warnings']:
                        result['uv_warnings'] = uv_validation['warnings']
                        result['warnings'].extend(uv_validation['warnings'])
                        
                        # Fix UV coordinates if needed
                        uv_fix_result = fix_uv_coordinates(output_path)
                        if uv_fix_result['success'] and uv_fix_result['fixed_count'] > 0:
                            result['uv_fixes'] = uv_fix_result['fixed_count']
                            logger.info(f"Fixed {uv_fix_result['fixed_count']} UV coordinate sets")
            
            # Step 5: Post-validation pass
            if result['success']:
                # Fast validation first
                validation_result = self._validate_output_fast(result.get('output_path'))
                result['validation'] = validation_result
            ...
            # Step 6: Cross-platform consistency checks
            if result['success'] and result.get('output_path'):
                output_path = result.get('output_path')
                if output_path and Path(output_path).exists():
                    # Apply cross-platform fixes
                    consistency_result = self.cross_platform_manager.fix_cross_platform_issues(Path(output_path))
                    if consistency_result['fixes_applied']:
                        result['cross_platform_fixes'] = consistency_result['fixes_applied']
                        logger.info(f"Applied {len(consistency_result['fixes_applied'])} cross-platform fixes")
                    
                    # Generate consistency report
                    consistency_report = self.cross_platform_manager.generate_consistency_report(
    ...
    def _process_complex_file(self, input_path: Path, output_dir: Path, target: str, 
        ...
        try:
            ...
            # Build command arguments
            # Check if this is our bundled voxbridge_node.js
            if node_runner_path.name == 'voxbridge_node.js':
                # Use direct command-line arguments for voxbridge_node.js
                cmd = [
                    'node',
                ...
                # Call process_complex.js directly with node
                process_complex_path = node_runner_path.parent / 'process_complex.js'
                if process_complex_path.exists():
                    cmd = [
                        'node',
                        str(process_complex_path),
                        '--input', str(input_path.absolute()),
                        '--output', str(output_dir.absolute()),
                        '--target', target
                    ]
                else:
                    # Fallback to index.js with subcommand
                    cmd = [
                        str(node_runner_path),
                        'process',
                        '--input', str(input_path.absolute()),
            ...
            if options.get('optimize_mesh', False):
                cmd.append('--simplify')
                simplify_ratio = options.get('simplify_ratio', 0.5)
            ...
            try:
                ...
                # Fallback: run without text mode and decode manually
                process_result = subprocess.run(
                    cmd,
                    capture_output=True,
                    cwd=node_runner_path.parent
                ...
                stdout = process_result.stdout.decode('utf-8', errors='replace') if process_result.stdout else ''
                stderr = process_result.stderr.decode('utf-8', errors='replace') if process_result.stderr else ''
                ...
                class ProcessResult:
                    def __init__(self, returncode, stdout, stderr):
                        ...
                        self.stdout = stdout
                        self.stderr = stderr
                process_result = ProcessResult(process_result.returncode, stdout, stderr)
    ...
    def _create_unified_package(self, output_path: str, output_dir: Path, target: str, model_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            with zipfile.ZipFile(package_path, 'w', zipfile.ZIP_DEFLATED) as zip_file:
                ...
                if target == 'unity':
                    unity_script_content = self._get_unity_import_script()
                    if unity_script_content:
                        zip_file.writestr('VoxbridgeImport.cs', unity_script_content)
                        package_info['files'].append('VoxbridgeImport.cs')
                        package_info['total_size'] += len(unity_script_content.encode('utf-8'))
                ...
                package_info['structure'] = {
                    ...
                    'report': 'report.json' in package_info['files'],
                    'unity_script': 'VoxbridgeImport.cs' in package_info['files']
                }
    ...
    def _check_node_availability(self) -> bool:
        """Check if Node.js is available (either system node or bundled node_runner)"""
        try:
            # First try bundled node_runner (preferred for portability)
            node_runner_path = get_node_runner_path()
            if node_runner_path.exists():
                # Ensure it's executable
                if ensure_executable(node_runner_path):
                    # Test if it works with a simple command
                    try:
                        result = subprocess.run([str(node_runner_path), '--version'], 
                                              capture_output=True, text=True, timeout=10)
                        if result.returncode == 0:
                        ...
                        logger.warning("Bundled Node.js runner timed out")
                    except Exception as e:
                        logger.warning(f"Bundled Node.js runner test failed: {e}")
            
            # Try alternative bundled locations
            alternative_paths = [
                Path(__file__).parent.parent / 'build' / 'node_runner',
                Path(__file__).parent.parent / 'build' / 'node_runner_comprehensive',
                Path(__file__).parent.parent / 'dist' / 'node_runner',
                Path(__file__).parent.parent / 'dist' / 'node_runner_comprehensive'
            ]
            
            for alt_path in alternative_paths:
                if alt_path.exists() and ensure_executable(alt_path):
                    try:
                        result = subprocess.run([str(alt_path), '--version'], 
                                              capture_output=True, text=True, timeout=10)
                        if result.returncode == 0:
                            logger.info(f"Using alternative bundled Node.js runner: {alt_path}")
                            return True
                    except Exception:
                        continue
            
            # Fallback to system node (less reliable for portability)
            try:
                result = subprocess.run(['node', '--version'], capture_output=True, text=True, timeout=5)
                if result.returncode == 0:
                    logger.info("Using system Node.js (fallback)")
                    return True
            except (FileNotFoundError, subprocess.TimeoutExpired):
                pass
                
            logger.warning("No Node.js runtime available - will use Python fallback")
            return False
            
        except Exception as e:
            logger.warning(f"Node.js availability check failed: {e}")
            return False
    
    def _get_unity_import_script(self) -> str:
        """Get Unity import script content with enhanced animation workflow"""
...
/// <summary>
/// VoxBridge Unity Import Script v2.0
/// Enhanced animation workflow with automatic clip naming and controller creation
/// </summary>
[System.Serializable]
public class VoxbridgeImport : AssetPostprocessor
{
    ...
    void OnPreprocessModel()
    {
        ...
        if (IsVoxbridgeModel(assetImporter.assetPath))
        {
            ...
            if (modelImporter != null)
            {
                ...
                modelImporter.animationPositionError = 0.001f;
                modelImporter.animationRotationError = 0.001f;
                modelImporter.animationScaleError = 0.001f;
                ...
                // Enable texture import optimization
    ...
    void OnPostprocessModel(GameObject g)
    {
        if (IsVoxbridgeModel(assetImporter.assetPath))
        {
            ...
            // Auto-setup animations with enhanced workflow
            SetupAnimations(g);
            
            // Auto-setup materials with texture optimization
            SetupMaterials(g);
    ...
    private void SetupAnimations(GameObject root)
    {
        ...
        // Auto-name animation clips based on common patterns
        AutoNameAnimationClips(clips);
        
        // Create Animator Controller with enhanced workflow
        ...
        // Set up default state and transitions
        SetupAnimationTransitions(controller, clips);
        
        Debug.Log($"VoxBridge: Created enhanced Animator Controller at {controllerPath}");
    }
    
    private void AutoNameAnimationClips(AnimationClip[] clips)
    {
        // Common animation naming patterns
        string[] commonNames = { "idle", "walk", "run", "jump", "attack", "death", "hit", "idle_01", "walk_01", "run_01" };
        ...
        for (int i = 0; i < clips.Length; i++)
        {
            ...
            if (!string.IsNullOrEmpty(clip.name) && !clip.name.StartsWith("Animation_"))
            {
                continue;
            }
            
            // Try to assign common names based on duration and properties
            if (i < commonNames.Length)
            {
                clip.name = commonNames[i];
            }
            else
            {
                // Generate descriptive names based on clip properties
                float duration = clip.length;
                if (duration < 1.0f)
                {
                    clip.name = $"quick_action_{i}";
                }
                else if (duration < 3.0f)
                {
                    clip.name = $"short_action_{i}";
                }
                else
                {
                    clip.name = $"long_action_{i}";
                }
            }
            
            Debug.Log($"VoxBridge: Renamed animation clip to '{clip.name}'");
        }
    }
    ...
    private AnimatorController CreateAnimatorController(string path, AnimationClip[] clips)
    {
        ...
        foreach (AnimationClip clip in clips)
        {
            ...
            else if (defaultState == null && clip == clips[0])
            {
                defaultState = state;
            }
        }
    ...
    private void SetupAnimationTransitions(AnimatorController controller, AnimationClip[] clips)
    {
        AnimatorControllerLayer baseLayer = controller.layers[0];
        AnimatorStateMachine stateMachine = baseLayer.stateMachine;
        
        // Create transitions between common animation states
        AnimatorState idleState = FindStateByName(stateMachine, "idle");
        AnimatorState walkState = FindStateByName(stateMachine, "walk");
        AnimatorState runState = FindStateByName(stateMachine, "run");
        
        if (idleState != null && walkState != null)
        {
            AnimatorStateTransition transition = idleState.AddTransition(walkState);
            transition.AddCondition(AnimatorConditionMode.If, 0, "Speed");
        ...
        {
            AnimatorStateTransition transition = walkState.AddTransition(runState);
            transition.AddCondition(AnimatorConditionMode.Greater, 1.0f, "Speed");
            transition.hasExitTime = false;
            transition.exitTime = 0.9f;
        }
    }
    ...
    {
        foreach (AnimatorState state in stateMachine.states)
        {
            if (state.state.name.ToLower().Contains(name.ToLower()))
            {
                return state.state;
            }
        }
        ...
        foreach (Renderer renderer in renderers)
        {
            ...
            for (int i = 0; i < materials.Length; i++)
            {
                if (materials[i] != null)
                {
                    ...
                    OptimizeMaterialTextures(materials[i]);
                }
            }
        }
    ...
    private void OptimizeMaterialTextures(Material material)
    {
        // Optimize texture import settings for VoxBridge materials
        string[] textureProperties = { "_MainTex", "_MetallicGlossMap", "_BumpMap", "_EmissionMap", "_OcclusionMap" };
        
        foreach (string prop in textureProperties)
        {
            if (material.HasProperty(prop))
            {
                Texture2D texture = material.GetTexture(prop) as Texture2D;
                if (texture != null)
                {
    ...
    private void CreateAnimationMetadata(GameObject root)
    {
        // Create a metadata component with animation information
        VoxbridgeAnimationMetadata metadata = root.AddComponent<VoxbridgeAnimationMetadata>();
        
        AnimationClip[] clips = AnimationUtility.GetAnimationClips(root);
        metadata.animationClips = clips;
        metadata.clipCount = clips.Length;
        
        // Log animation metadata
        Debug.Log($"VoxBridge: Created animation metadata with {clips.Length} clips");
        foreach (AnimationClip clip in clips)
        {
...
/// <summary>
/// Component to store animation metadata
/// </summary>
public class VoxbridgeAnimationMetadata : MonoBehaviour
{
    ...
    void Start()
    {
        Debug.Log($"VoxBridge Animation Metadata: {clipCount} animation clips available");
    }
}'''
