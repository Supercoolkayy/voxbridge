...
# Import robust converter
try:
    from .robust_converter import RobustConverter
    ROBUST_CONVERTER_AVAILABLE = True
except ImportError:
    ROBUST_CONVERTER_AVAILABLE = False
...
class VoxBridgeConverter:
    ...
    def __init__(self, debug: bool = False):
        ...
        if ROBUST_CONVERTER_AVAILABLE:
            self.robust_converter = RobustConverter()
        else:
            self.robust_converter = None
            if debug:
                print("Robust converter not available")
        ...
        try:
            ...
            try:
                ...
                if gltf_output.exists():
                    ...
                    if zip_path.suffix == '.zip':
                        # Remove all standalone GLTF files since we only want the ZIP
                        try:
                            # Remove the original GLTF file
                            original_gltf = output_path.with_suffix('.gltf')
                            if original_gltf.exists():
                                original_gltf.unlink()
                                if self.debug:
                                    print(f"Removed original GLTF file: {original_gltf.name}")
                            
                            # Remove the platform-specific GLTF file
                            if gltf_output.exists():
                                gltf_output.unlink()
                                if self.debug:
                                    print(f"Removed platform GLTF file: {gltf_output.name}")
                        except Exception as e:
                            if self.debug:
                                print(f"Warning: Could not remove standalone GLTF files: {e}")
                        
                        print(f"Conversion complete. Your files are packaged into {zip_path.name}")
                        print(f"📦 Unzip the folder to use in your {platform} project!")
                    else:
    ...
    def convert_file(self, input_path: Path, output_path: Path, use_blender: bool = True, optimize_mesh: bool = False, generate_atlas: bool = False, compress_textures: bool = False, platform: str = "unity") -> bool:
        ...
        if not self.validate_input(input_path):
            ...
            print("   File must be a .glb or .gltf file.")
            return False
        ...
        if use_blender and self._can_use_blender(input_path):
            try:
                ...
                if self.convert_with_blender(input_path, output_path, optimize_mesh=optimize_mesh, platform=platform):
                    ...
                    print("⚠️  Blender conversion failed. Trying Assimp...")
            except Exception as e:
                if self.debug:
        ...
        try:
            result = self.convert_gltf_json(input_path, output_path, generate_atlas=generate_atlas, compress_textures=compress_textures, platform=platform)
            if not result:
                print("❌ Basic converter also failed. Check the input file format and try again.")
                print("   Possible issues:")
                print("   - File is corrupted or not a valid GLB/GLTF file")
                print("   - File contains unsupported features")
                print("   - File is too large or complex")
            return result
        except Exception as e:
            print(f"❌ Basic converter failed with error: {e}")
            print("   This usually indicates a corrupted or invalid input file.")
            return False
    ...
    def convert_gltf_json(self, input_path: Path, output_path: Path, generate_atlas: bool = False,
        ...
        try:
            ...
            if gltf_output.exists():
                zip_path = self._package_output_files(output_path, gltf_output)
                if zip_path.suffix == '.zip':
                    # Remove the standalone GLTF file since we only want the ZIP
                    try:
                        gltf_output.unlink()
                        if self.debug:
                            print(f"Removed standalone GLTF file: {gltf_output.name}")
                    except Exception as e:
                        if self.debug:
                            print(f"Warning: Could not remove standalone GLTF file: {e}")
        ...
        try:
            ...
            for old_file in output_dir.glob(f"{output_stem}*"):
                if old_file != output_path:  # Don't delete the target file
                    # Only delete files that are clearly output files, not input files
                    if old_file.suffix in ['.gltf', '.zip', '.bin', '.json']:
                        old_file.unlink()
                        if self.debug:
                            print(f"Cleaned up old output file: {old_file.name}")
            
            # DO NOT delete .glb files as they might be input files!
            # Only clean up specific output patterns
            for old_gltf in output_dir.glob(f"{output_stem}_*.gltf"):
                if old_gltf != output_path:
                    old_gltf.unlink()
                    if self.debug:
                        print(f"Cleaned up old GLTF file: {old_gltf.name}")
...
# Try to import texture optimization modules (optional)
try:
    from .texture_optimizer import resize_texture, generate_texture_atlas, update_gltf_with_atlas, ensure_texture_embedding, flatten_texture_paths
    TEXTURE_OPTIMIZATION_AVAILABLE = True
except ImportError:
    TEXTURE_OPTIMIZATION_AVAILABLE = False
...
# Import robust converter (optional)
try:
    from .robust_converter import RobustConverter
    ROBUST_CONVERTER_AVAILABLE = True
except ImportError:
    ROBUST_CONVERTER_AVAILABLE = False
    # Create a simple fallback robust converter
...
class VoxBridgeConverter:
    ...
    def __init__(self, debug: bool = False):
        ...
        if ROBUST_CONVERTER_AVAILABLE:
            ...
            self.robust_converter = RobustConverter()  # Use fallback
            if debug:
                print("Using fallback robust converter")
    ...
    def apply_texture_optimizations(self, gltf_path: Path, platform: str) -> bool:
        """Apply comprehensive texture optimizations with consistent handling"""
        try:
            if not TEXTURE_OPTIMIZATION_AVAILABLE:
                return False
            
            # Ensure texture embedding and fix paths (consistent with Node.js route)
            texture_embedding_result = ensure_texture_embedding(gltf_path, gltf_path.parent)
            if texture_embedding_result['success'] and self.debug:
                print(f"Texture embedding: {texture_embedding_result['message']}")
            
            # Flatten texture paths for Unity/Roblox compatibility (consistent with Node.js route)
            texture_flatten_result = flatten_texture_paths(gltf_path, gltf_path.parent)
            if texture_flatten_result['success'] and self.debug:
                print(f"Texture path flattening: {texture_flatten_result['message']}")
